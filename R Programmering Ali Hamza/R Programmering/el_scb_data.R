...
# DATA MANIPULATION
3---------------------------------------------------------------------------------------------------
scb_fordon <- px_data_frame

scb_fordon$drivmedel <- NULL
View(scb_fordon)

scb_fordon$År <- as.numeric(substr(scb_fordon$månad, 1, 4))  # Extract year (first 4 characters)
scb_fordon$Månad <- as.numeric(substr(scb_fordon$månad, 6, 7))  # Extract month (characters 6 and 7)


scb_fordon$månad <- as.yearmon(scb_fordon$månad,"%YM%m")


colnames(scb_fordon)[colnames(scb_fordon) == 'Nyregistrerade personbilar'] <- 'Nyregistrerat_personbilar'

cars_by_year <- aggregate(Nyregistrerat_personbilar ~ År, scb_fordon, sum)
print(cars_by_year)

cars_by_year2 <- aggregate(Miltal ~ Pris, car_data_fixed, sum)
print(order_sales)
order_sales <- cars_by_year2[order(cars_by_year2$Miltal), ]
#---------------------------------------------------------------------------------------------------
# PLOTTING
#---------------------------------------------------------------------------------------------------
plot4 <- ggplot(cars_by_year_noel, aes(x = År, y = Nyregistrerat_personbilar, fill = drivmedel)) +
...
plot5 <- ggplot(cars_by_year, aes(x = År, y = Nyregistrerat_personbilar)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "New Electric Car OWners Per | y Year",
       x = "Year",
       y = "New Car Owners",
       ) +
  theme_minimal()

grid.arrange(plot1, plot5, ncol = 1)
